...
| Source    | df       | SS                                                  | MS                                                        | F                                     |
|-----------|----------|-----------------------------------------------------|-----------------------------------------------------------|---------------------------------------|
| Treatment | $k-1$    | $SS_{treatment} = n\Sigma(\bar{X}_{j}-\bar{X}..)^2$ | $MS_{treatment} = \frac{SS_{treatment}}{df_{treatment}}$ | $F = \frac{MS_{treatment}}{MS_{error}}$ |
| Error     | $k(n-1)$ | $SS_{error} = \Sigma(X_{ij}-\bar{X}_j)^2$           | $MS_{error} = \frac{SS_{error}}{df_{error}}$             |                                       |
| Total     | $N-1$    | $SS_{total} = SS_{treatment} + SS_{error}$          |                                                           |                                       |
...
>-   Since our obtained F > critical F we can reject the null hypothesis.

### Worked Example (from study guide) {.well}

#### Data

The effects of temperature on cognitive processes

```{r echo=TRUE}
tempData <- read.csv("data/tempData.csv")
colnames(tempData) <- c("10 degrees", "20 degrees", "30 degrees")

treatMean10 <- round(mean(tempData$`10 degrees`), 2)
treatMean20 <- round(mean(tempData$`20 degrees`), 2)
treatMean30 <- round(mean(tempData$`30 degrees`), 2)
grandMean <- round((treatMean10 + treatMean20 + treatMean30)/ncol(tempData), 2)

SStreat <- round(nrow(tempData) * (((treatMean10 - grandMean)^2) + ((treatMean20 - grandMean)^2) + ((treatMean30 - grandMean)^2)),1)



for(i in 1:ncol(tempData)) {
  SSerror <- sum((tempData[ , i] - mean(tempData$'10 degrees'))^2)
}
...
>$\bar{X}.. =$ grand mean
>
> $SS_{treatment} &= n\Sigma(\bar{X}_{j}-\bar{X}..)^2 =$ `{r} SStreat`
